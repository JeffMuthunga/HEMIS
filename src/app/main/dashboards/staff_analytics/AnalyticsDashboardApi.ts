...
import VisitorsVsPageViewsType from "./widgets/types/enrollmentGrowthType";
...
declare module "app/store/lazyLoadedSlices" {
  export interface LazyLoadedSlices
    extends WithSlice<typeof AnalyticsDashboardApi> {}
}

export const selectWidget = <T>(id: string) =>
  rootReducer.selector((state) => {
    const widgets =
      AnalyticsDashboardApi.endpoints.getAnalyticsDashboardWidgets.select()(
        state
      )?.data;
